...
Uses OAuth token from file descriptor if ANTHROPIC_API_KEY not set.
"""
...
def get_api_client():
    ...
    if not api_key:
        # Try reading from the token passed as command-line argument
        if len(sys.argv) > 1:
            api_key = sys.argv[1].strip()
    if not api_key:
        raise RuntimeError("No API key found. Pass it as first argument.")
