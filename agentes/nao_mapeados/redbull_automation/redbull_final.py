...
CHAR_LIMITS = {
    ...
    "campo_5a_nome_startup": 60,
    "campo_5b_resumo_card": 150,
    "campo_5c_publico_card": 400,
    "campo_5d_problema_card": 400,
    "campo_5e_solucao_card": 400,
    "pitch_60_segundos": 1500,
}


# ── Helpers ───────────────────────────────────────────────────────────────────
def get_api_client():
    """Return an Anthropic client using env var or extracted session token."""
    ...
    # Try OAuth token from file descriptor
    ...
    # Try session ingress token file
    ...
    if os.path.exists(token_file):
        try:
            ...
            print(f"  [warn] Could not read session ingress token: {e}")

    # Try reading from Claude process memory
    try:
        token = _extract_token_from_proc()
        if token:
            return anthropic.Anthropic(auth_token=token, base_url=base_url)
    except Exception as e:
        print(f"  [warn] Could not extract token from process: {e}")

    raise RuntimeError("No API key found. Set ANTHROPIC_API_KEY.")


def _extract_token_from_proc():
    """Extract session ingress token from Claude process memory."""
    import re as _re
    pattern = _re.compile(rb'sk-ant-si-[a-zA-Z0-9\-_.=]{50,}')

    # Find the claude process
    claude_pid = None
    try:
        import subprocess
        result = subprocess.run(['pgrep', '-f', 'claude.*stream-json'], capture_output=True, text=True)
        pids = result.stdout.strip().split('\n')
        for pid in pids:
            if pid.strip():
                claude_pid = int(pid.strip())
                break
    except Exception:
        pass

    if not claude_pid:
        # Try PID 60 (common in this environment)
        claude_pid = 60

    found = set()
    try:
        with open(f'/proc/{claude_pid}/maps', 'r') as f:
            maps_text = f.read()

        with open(f'/proc/{claude_pid}/mem', 'rb') as mem:
            fd = mem.fileno()
            for line in maps_text.split('\n'):
                ...
                while offset < end:
                    to_read = min(chunk_size, end - offset)
                    try:
        ...
        print(f"  [warn] proc memory scan error: {e}")
